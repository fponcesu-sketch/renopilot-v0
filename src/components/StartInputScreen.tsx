...
const labels = {
  ...
  en: {
    ...
    helpers: [
      ...
      'Write what you remember from the call. We prepare a message to request written confirmation.',
    ],
    placeholder: 'Example: mosquito screens for 4 windows, 1200 PLN, installation included, next week.',
    more: 'Add more context if you want.',
  ...
  pl: {
    ...
    helpers: [
      ...
      'Napisz, co pamiętasz z rozmowy. Przygotujemy wiadomość z prośbą o potwierdzenie.',
    ],
    placeholder: 'Np. moskitiery do 4 okien, 1200 PLN, montaż w cenie, w przyszłym tygodniu.',
    more: 'Dodaj więcej kontekstu, jeśli chcesz.',
...
export function StartInputScreen({ error, language, onSubmit }: Props) {
  ...
  const submit = () => {
    ...
    if (mode !== 'verbal_estimate' && !hasReadableText(documents, text)) {
      setLocalError(t.unreadablePdf);
      return;
    }
  ...
  return (
    <form className="screen-content form-screen start-check-screen simple-input-screen" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); submit(); }}>
      ...
      <div className="input-pill-row" aria-label={t.title}>
        {modes.map((pillMode, index) => (
          <button
            aria-pressed={selectedMode === pillMode}
            className={selectedMode === pillMode ? 'input-pill active' : 'input-pill'}
            key={pillMode}
            onClick={() => { setSelectedMode((current) => current === pillMode ? null : pillMode); setLocalError(''); }}
            type="button"
          >
            {t.pills[index]}
          </button>
      ...
      {documents.length > 0 && (
        <div className="attached-file-list compact-attached-list">
          {documents.map((document, index) => (
            <div className="attached-file-pill" key={`${document.name}-${index}`}>
              <span>✓ {document.mimeType?.startsWith('image/') ? t.imageAttached : t.attached}</span>
